...
class ParkingService:
    def __init__(
        self, capacity=50, hourly_rate=120.0
    ):  # self je instanca ParkingService
    ...
    def show_parked_vehicles(self):
        ...
        for ticket in self.__tickets:
            ...
            print(
                f"Id: {ticket.get_id()} | Reg: {ticket.get_vehicle_registration()} | Entry: {time_str} | Exit: {time_now_str} | Barcode: {ticket.get_bar_code()}"
            )
...
if __name__ == "__main__":  # Main entry point for the simulation
    ...
    for _ in range(vehicle_count):
        ...
        suffix = random.choice(vehicle_suffixes)
        number = random.randint(1000, 9999)  # Random number of vehicles
        registration = f"{prefix}{number}{suffix}"
        garage.enter_vehicle(registration)
